...
def run_funnel_single_slice(
    ...
    # ---------------- ANCHOR: 90% on best model (still ensemble) ----------------
    # 触发条件：best 比 second 明显好（gap_trigger），则强制 best:0.90 + second:0.10
    # 然后再检查一次 val MAPE 不变差，否则退回 best 单模型
    try:
        anchor_alpha = float(config.get("anchor_alpha", 0.90))         # 0.90 => 9成
        gap_trigger = float(config.get("anchor_gap_trigger", 0.03))    # second-best 至少比 best 高 0.03 才触发

...
class AnalysisAgent:
    ...
    def _profile_data(data: pd.DataFrame) -> Dict[str, Any]:
        ...
        # Stationarity: ADF + KPSS (safe try)
        try:
            if len(vals) >= 20:
                ...
                profile["adf_pvalue"] = None
        except Exception:
            profile["adf_pvalue"] = None

        try:
            if len(vals) >= 20:
                kpss_stat, kpss_p, *_ = kpss(vals, nlags="auto")
                profile["kpss_statistic"] = float(kpss_stat)
                profile["kpss_pvalue"] = float(kpss_p)
            else:
                profile["kpss_pvalue"] = None
        except Exception:
            profile["kpss_pvalue"] = None

        adf_ok = profile.get("adf_pvalue") is not None and profile["adf_pvalue"] < 0.05
        kpss_ok = profile.get("kpss_pvalue") is not None and profile["kpss_pvalue"] > 0.05
        profile["is_stationary"] = bool(adf_ok and (kpss_ok if profile.get("kpss_pvalue") is not None else True))
        ...
        # Simple seasonality / dependence probes
        for lag in (7, 12, 24):
            if len(vals) > lag:
                try:
                    x = vals[:-lag]
                    ...
                    if np.std(x) > 0 and np.std(y) > 0:
                        profile[f"acf_lag_{lag}"] = float(np.corrcoef(x, y)[0, 1])
                    else:
                        profile[f"acf_lag_{lag}"] = 0.0
                except Exception:
                    profile[f"acf_lag_{lag}"] = None
            else:
                profile[f"acf_lag_{lag}"] = None

        try:
            if len(vals) >= 30:
                lb = acorr_ljungbox(vals, lags=[min(24, max(5, len(vals) // 10))], return_df=True)
                profile["ljung_box_pvalue"] = float(lb["lb_pvalue"].iloc[-1])
            else:
                profile["ljung_box_pvalue"] = None
        except Exception:
            profile["ljung_box_pvalue"] = None
...
"""
    ...
    def _rule_based_constraints(data_profile: Dict[str, Any]) -> Dict[str, Any]:
        ...
        n = int(data_profile.get("n", 0) or 0)
        slope = float(data_profile.get("trend_slope", 0.0) or 0.0)
        is_stationary = data_profile.get("is_stationary")

        # Very short series -> avoid deep models and heavy Fourier
        if n < 64:
            constraints["forbidden_L3_models"].append("deep")
            constraints["forbidden_L2_actions"].append("fourier")

        # Already stationary -> discourage extra differencing
        if is_stationary is True:
            constraints["forbidden_L1_actions"].append("diff")

        # Strong trend / non-stationary -> 'none' stationarity is risky for pure statistical models
        if is_stationary is False and abs(slope) > max(1e-6, 0.01 * float(data_profile.get("std", 1.0) or 1.0)):
            constraints["forbidden_L3_models"].append("ARIMA")

        # Very weak seasonality evidence -> Fourier usually not worth it
        acf24 = data_profile.get("acf_lag_24")
        acf12 = data_profile.get("acf_lag_12")
        acf7 = data_profile.get("acf_lag_7")
        seasonal_strength = max([abs(v) for v in (acf24, acf12, acf7) if isinstance(v, (int, float))], default=0.0)
        ...
        for k in constraints:
            seen = set()
            deduped = []
            for x in constraints[k]:
                if x not in seen:
                    seen.add(x)
                    deduped.append(x)
            constraints[k] = deduped
